...
pip uninstall colorama langchain langchain-community langchain-openai python-dotenv # if pip doesn't work, try pip3

# Check if the psql command was successful
if [ $? -eq 0 ]; then
    echo "Python dependencies uninstalled successfully"
else
    echo "Failed to uninstall langchain, Please see the error message above"
fi